...
namespace AircraftModel
{
    class Aircraft
    {
        ...
        public double Get_Takeoff_Distance(int numberOfEngines, double h = 0,
            double TOGA_ClimbEngineRatio = 1.1, double takeoffEngineLapseRate = 0.864,
            double transitionEngineLapseRate = 0.803, 
            TakeoffRunwayCondition takeoffRunwayCondition = TakeoffRunwayCondition.Paved, 
            double n = 1.2, double screenHeight = 35,
            double DeltaISA = 0)
        {
            return Get_Takeoff_Distance(numberOfEngines, m_max, h, TOGA_ClimbEngineRatio,
                takeoffEngineLapseRate, transitionEngineLapseRate, takeoffRunwayCondition, n,
            ...
            double n = 1.2, double screenHeight = 35, double DeltaISA = 0)
        {
            ...
            double T_TOGA = TOGA_ClimbEngineRatio * numberOfEngines * Get_T(h, v_LOF, 
                FlightPhase.Climb, m: takeoffWeight, DeltaISA: DeltaISA);
            double T = takeoffEngineLapseRate * T_TOGA;
            double W = takeoffWeight * AtmosphereEnviroment.g;
            double K_T = T / W - mu;
            double rho = AtmosphereEnviroment.Get_rho(h, DeltaISA);
            double C_L = Get_C_L(h, Units.kt2mps(v_stall_TO), DeltaISA: DeltaISA) * 0.54;
            double C_D = C_D0_CR + C_D2_CR * C_L * C_L;
            double K_A = rho * (mu * C_L - C_D) / (2 * W / S);
            double S_G = 1 / (2 * AtmosphereEnviroment.g * K_A / 2.5) * Math.Log((K_T +
                K_A * v_LOF * v_LOF) / K_T);
            ...
            T = transitionEngineLapseRate * T_TOGA;
            double gamma = (T - D) / W;
            double S_T = r * gamma;
        ...
        public double Get_Balanced_Field_Length(int numberOfEngines, double v_1,
            ...
            double TOGA_ClimbEngineRatio = 1.1, double takeoffEngineLapseRate = 0.864, 
            double accelerationEngineLapseRate = 0.66, double transitionEngineLapseRate = 0.655, 
            TakeoffRunwayCondition takeoffRunwayCondition = TakeoffRunwayCondition.Paved,
            double n = 1.2, double screenHeight = 35, double DeltaISA = 0)
        {
            return Get_Balanced_Field_Length(numberOfEngines, v_1, balancedFieldLengthType,
                m_max, h, TOGA_ClimbEngineRatio, takeoffEngineLapseRate, 
                accelerationEngineLapseRate, transitionEngineLapseRate,
        ...
        public double Get_Balanced_Field_Length(int numberOfEngines, double v_1,
            ...
            double h = 0, double TOGA_ClimbEngineRatio = 1.1, double takeoffEngineLapseRate = 0.864, 
            double accelerationEngineLapseRate = 0.66, double transitionEngineLapseRate = 0.655,
            TakeoffRunwayCondition takeoffRunwayCondition = TakeoffRunwayCondition.Paved,
            double n = 1.2, double screenHeight = 35, double DeltaISA = 0)
        {
            ...
            double T = TOGA_ClimbEngineRatio * takeoffEngineLapseRate * numberOfEngines * 
                Get_T(h, v_1, FlightPhase.Climb, DeltaISA: DeltaISA);
            double W = takeoffWeight * AtmosphereEnviroment.g;
            double K_T = T / W - mu;
            double rho = AtmosphereEnviroment.Get_rho(h, DeltaISA);
            double C_L = Get_C_L(h, Units.kt2mps(v_stall_TO), DeltaISA: DeltaISA) * 0.54;
            ...
            T = TOGA_ClimbEngineRatio * accelerationEngineLapseRate * (numberOfEngines - 1) * 
                Get_T(h, v_m, FlightPhase.Climb, DeltaISA: DeltaISA);
            K_T = T / W - mu;
            C_D += C_D_windmilling + C_D_asymmetric;
            K_A = rho * (mu * C_L - C_D) / (2 * W / S);
            ...
            T = transitionEngineLapseRate * (numberOfEngines - 1) * Get_T(h, v_m, FlightPhase.Climb,
                DeltaISA: DeltaISA);
            C_L = Get_C_L(h, v_LOF, DeltaISA: DeltaISA);
            ...
            {
                T = TOGA_ClimbEngineRatio * 0.864 * numberOfEngines * Get_T(h, v_1, 
                    FlightPhase.Climb, DeltaISA: DeltaISA);
                W = takeoffWeight * AtmosphereEnviroment.g;
                K_T = T / W - mu;
                rho = AtmosphereEnviroment.Get_rho(h, DeltaISA);
                C_L = Get_C_L(h, Units.kt2mps(v_stall_TO), DeltaISA: DeltaISA) * 0.54;
                C_D = C_D0_CR + C_D2_CR * C_L * C_L;
                K_A = rho * (mu * C_L - C_D) / (2 * W / S);
                S_G = 1 / (2 * AtmosphereEnviroment.g * K_A / 2.2) * Math.Log((K_T +
                    K_A * v_1 * v_1) / K_T);
